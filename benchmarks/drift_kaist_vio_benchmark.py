...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark compare_repos/drift on KAIST VIO.")
    ...
    parser.add_argument("--output-dir", default=str(PROJECT_ROOT / "outputs" / "benchmarks" / "drift_kaist_vio"))
    ...
    parser.add_argument(
        ...
        help="Velocity correction noise std for the drift-native comparison path.",
    )
    ...
    if pose_type != "3d":
        raise ValueError("drift KAIST VIO benchmark expects 3D data.")
    ...
    _write_metadata(output_dir / "metadata.json", args, dataset_name, dataset_csv, input_csv, results, results_csv, metric_plot_paths, animation_outputs)

    print(f"[DriftBenchmark] Dataset CSV : {dataset_csv}")
    print(f"[DriftBenchmark] Our 9D CSV : {filter_input_csv}")
    print(f"[DriftBenchmark] Our 15D CSV: {filter_input_15d_csv}")
    print(f"[DriftBenchmark] Repo input  : {input_csv}")
    print(f"[DriftBenchmark] Results     : {results_csv}")
...
def _run_drift(args: argparse.Namespace, input_csv: Path, estimates_csv: Path, gt: np.ndarray) -> BenchmarkResult:
    ...
    if runner is None:
        return _skipped(len(gt), "no runner executable found; pass --runner after building a drift KAIST runner")
    ...
    return BenchmarkResult("external", "drift", "inekf", "ok", rmse, variance, runtime, len(gt), estimates=estimates, input_csv=str(input_csv))
...
def _resolve_runner(configured: str) -> Path | None:
    ...
    candidates.extend(
        [
            ...
            COMPARE_REPO / "build" / "drift_kaist_vio_runner",
        ]
    )
...
def _with_velocity_measurements(
    dataset: list[dict],
    gt: np.ndarray,
    ...
    for idx, sample in enumerate(dataset):
        ...
        corrected_control = control.copy()
...
def _estimate_world_velocity(timestamps_ns: np.ndarray, positions: np.ndarray) -> np.ndarray:
    ...
    timestamps_s = np.asarray(timestamps_ns, dtype=float).reshape(-1) * 1e-9
...
def _write_filter_input_csv(path: Path, dataset: list[dict], gt: np.ndarray, timestamps_ns: np.ndarray) -> Path:
    ...
    with path.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        for idx, sample in enumerate(dataset):
            ...
            velocity = np.asarray(velocity_measurement if has_velocity else [np.nan, np.nan, np.nan], dtype=float).reshape(3)
            writer.writerow(
                {
                    "step": idx,
...
def _write_metadata(
    ...
    data = {
        ...
        "pseudo_position_offset": max(0, int(args.pseudo_position_offset)),
        "position_measurement_noise_std": list(args.position_measurement_noise_std),
        "velocity_measurement_noise_std": list(args.velocity_measurement_noise_std),
...
def _skipped(steps: int, notes: str) -> BenchmarkResult:
    return BenchmarkResult("external", "drift", "inekf", "skipped", None, None, None, steps, notes)


def _failed(steps: int, runtime: float, notes: str) -> BenchmarkResult:
    return BenchmarkResult("external", "drift", "inekf", "failed", None, None, runtime, steps, notes)
